...
export class Product3Component implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.finkartService.getAllProducts().subscribe(data=>{
      this.products=data;
      this.product.productName=this.products[2].productName;//change this only
      this.product.productId=this.products[2].productId;//change this only
      this.product.productRate=this.products[2].productRate;//change this only
    });
